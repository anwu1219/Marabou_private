...
class MarabouNetworkONNXConcat(MarabouNetwork.MarabouNetwork):
    ...
    def processGraph(self):
        ...
        for node in self.graph.input:
            self.shapeMap[node.name] = list([dim.dim_value if (dim.dim_value > 0 and dim.dim_value != 20) else 1 for dim in node.type.tensor_type.shape.dim])
    ...
    def reshape(self, node):
        ...
        if reshapeVals[0] == 20:
            reshapeVals = (1, reshapeVals[1], reshapeVals[2],reshapeVals[3])
